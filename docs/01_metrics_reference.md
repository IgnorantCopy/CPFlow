...
| 8 | **count(ΔpLDDT > 10)** | 单残基 pLDDT 差异 > 10 的位点数量 | ≤ 均值 + 1σ | 🟡 numpy 计算，依赖 #6 |
| 9 | **RMSD** | `√(1/N · Σ‖r_AP − r_WT‖²)`，CA 原子坐标均方根偏差 | < 3.0Å (KmAgo) / < 1.0Å (PfAgo) | 🟢 US-align subprocess |
| 10 | **TM-score** | 0-1，全局折叠相似性（>0.5 同折叠，>0.9 几乎相同） | > 0.9 (KmAgo) / > 0.97 (PfAgo) | 🟢 US-align subprocess（与 #9 同一次调用） |

**筛选逻辑链**：整体 pLDDT → σ(ΔpLDDT) → count(ΔpLDDT>10) → 残基级精细对比 → RMSD + TM-score 确认。论文中 ~30% 的生成序列通过筛选。
...
| 13 | **催化基序保留** | PIWI 结构域催化四联体 DEDD/DEDH 四个残基是否全部正确 | **100%——不可妥协的底线** | 🟡 纯 Python ~15行 |
...
g++ -O3 -o USalign USalign.cpp
# 批量: for pdb in result/structures/*.pdb; do ./USalign $pdb WT.pdb ... done
...
large_diffs = np.sum(np.abs(delta) > 10)
...
| **US-align** | `g++ -O3 -o USalign USalign.cpp` | RMSD (#9), TM-score (#10) | `subprocess.run()` |
...
def compute_rmsd_tm(pdb1, pdb2):
    r = subprocess.run(['USalign', pdb1, pdb2, '-mm', '0', '-ter', '0'],
                       capture_output=True, text=True)
    tm  = float(re.search(r'TM-score=\s+([\d.]+)', r.stdout).group(1))
    rmsd = float(re.search(r'RMSD=\s+([\d.]+)', r.stdout).group(1))
    return rmsd, tm
```
