...
**关键解读**：
- `catalytic_motif.intact = true` → **必须通过**，任一失败 = 序列不可用
...
**关键解读**：
- `final_pass_rate` ~ 27-30% 论文中 KmAgo 的水平
...
|------|----------|:--:|------|
| 催化基序保留 (KmAgo) | 100% | 100% | **不可退化** |
| 催化基序保留 (PfAgo) | 100% | — | 使用 `--motif pfago --fix_pos_file dataset/Ago/pfago.piwi.fix.txt` |
| 恢复率 | — | 71-76% | ≥ 原方法 |
| 序列一致性 vs WT | 50-70% | 50-70% | 不超出范围 |
| AA 属性保留 | — | — | 电荷翻转率低 |
| 整体 pLDDT | ≥ μ−1σ | ~85 | 不显著低于原方法 |
| σ(ΔpLDDT) | ≤ μ+1σ | — | 不显著高于原方法 |
| RMSD | < 3.0Å | < 3.0Å | ≤ 原方法 + 0.5Å |
| TM-score (WT 归一) | > 0.9 | > 0.9 | ≥ 原方法 − 0.05 |
| 筛选通过率 | ~27-30% | ~27% | ≥ 原方法 × 0.7 |
