...
def get_stat(graph_root, limited_num=None, num_subgroup=1000, max_limits=100000):
    ...
    # initialize mean, std
    graph = torch.load(os.path.join(graph_root, filenames[0]), weights_only=False)
    ...
    for i in tqdm(range(n)):
        file = filenames[i]
        graph = torch.load(os.path.join(graph_root, file), weights_only=False)
        x, pos, mu_r_norm, edge_attr = graph.x, graph.pos, graph.mu_r_norm, graph.edge_attr
    ...
    filename = 'mean_attr'
    saved_filename_pt = os.path.join(os.path.dirname(graph_root), filename + '.pt')
    torch.save(dic, saved_filename_pt)
    saved_filename = os.path.join(os.path.dirname(graph_root), filename + '.csv')
    w = csv.writer(open(saved_filename, 'w'))
    for key, val in dic.items():
        w.writerow([key, val])

    saved_filename = os.path.join(os.path.dirname(graph_root), filename + '_proteins.txt')
    with open(saved_filename, 'w') as f:
        for i in range(n):
            f.write(str(filenames[i]) + '\n')

    saved_filename = os.path.join(os.path.dirname(graph_root), filename + '_wrong_proteins.txt')
    with open(saved_filename, 'w') as f:
